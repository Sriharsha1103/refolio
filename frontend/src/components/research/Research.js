...
function Research() {
    ...
    const handleDelete = (data) => {
        ...
        let confirm =window.confirm('This action will permenently delete '+data.title+' research project.')
        if(confirm){
            service.delete('api/research/data/'+data._id).then((res)=>{
    ...
    if(titles.length==0){
        ...
        service.get("api/research/data?title=" + publicationFilterValue + "&branch=" + branchFilterValue + "&authors=" + authorsFilterValue + "&co=" + publishedByFilterValue + "&scheme=" +nationalityFilterValue+"&year="+scopusFilterValue+ "&page=" + pageNo + "&limit=" + perPage ).then((json) => {
...
>
  Download me
                ...
                <MDBRow>
                    <MDBCol md="4">
                        <ResearchExportCSV csvData={data} fileName={"Research"} />
                    </MDBCol>
                    <MDBCol md="4" >
...
<table  ref={tableRef}>
                ...
                <ReactTableFixedColumns
                    ...
                    columns={[
                        ...
                        {
                            Header: () => (<div>{ResearchKey.title}<br /><input type="text" id="publication" name="publication" onKeyPress={(e) => { if (e.key === "Enter") { setGetApi(getApi + 1); setPageNo(1) } }} onChange={(e) => { setPublicationFilterValue(e.target.value); }} /></div>),
                            accessor: "title",
                            disableSortBy: true,
                        ...
                        {
                            Header: () => (<div>{ResearchKey.pi}<br /><input type="text" id="year" name="year" onKeyPress={(e) => { if (e.key === "Enter") { setGetApi(getApi + 1); setPageNo(1) } }} onChange={(e) => { setAuthorsFilterValue(e.target.value); }} /></div>),
                            accessor: "pi",
                        ...
                        {
                            Header: () => (<div>{ResearchKey.co_pi}<br /><input type="text" id="published_by" name="published_by" onKeyPress={(e) => { if (e.key === "Enter") { setGetApi(getApi + 1); setPageNo(1) } }} onChange={(e) => { setPublishedByFilterValue(e.target.value); }} /></div>),
                            accessor: "co_pi",
                        ...
                        {
                            Header: () => (<div>{ResearchKey.dept}<br /><input size="sm" type="text" id="branch" name="branch" onKeyPress={(e) => { if (e.key === "Enter") { setGetApi(getApi + 1); setPageNo(1) } }} onChange={(e) => { setBranchFilterValue(e.target.value); }} /></div>),
                            accessor: "dept",
                            Cell: (row) =>{
                        ...
                        {
                            Header: () => (<div>{ResearchKey.scheme}<br /><input type="text" id="nationality" name="nationality" onKeyPress={(e) => { if (e.key === "Enter") { setGetApi(getApi + 1); setPageNo(1) } }} onChange={(e) => { setNationalityFilterValue(e.target.value); }} /></div>),
                            accessor: "scheme",
                            // Cell: (row) => {
                            //     //   console.log(row)
                        ...
                        {
                            Header: () => (<div>{ResearchKey.year}<br /><input type="text" id="nationality" name="nationality" onKeyPress={(e) => { if (e.key === "Enter") { setGetApi(getApi + 1); setPageNo(1) } }} onChange={(e) => { setScopusFilterValue(e.target.value); }} /></div>),
                            accessor: "year",
                            getProps: (state, rowInfo, column) => {
                                return {
                                    style: {
                        ...
                        {
                            Header: () => (<div>{ResearchKey.duration}<br /><input type="text" id="nationality" name="nationality" onKeyPress={(e) => { if (e.key === "Enter") { setGetApi(getApi + 1); setPageNo(1) } }} onChange={(e) => { setScopusFilterValue(e.target.value); }} /></div>),
                            accessor: "duration",
                            getProps: (state, rowInfo, column) => {
                                return {
                                    style: {
                            ...
                            Cell: (row) => {
                                ...
                                return (
                                    <div className="button-group">
                                        ...
                                        <EditResearch edit={row.original} titles={titles} />
